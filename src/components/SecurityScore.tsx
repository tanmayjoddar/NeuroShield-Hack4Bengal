...
import { 
  Shield, TrendingUp, Award, Zap, Loader2, Lock, Unlock, Send, History, 
  AlertTriangle, CheckCircle2, Network, Activity, Share2, XCircle, 
  AlertOctagon, Brain 
} from 'lucide-react';
...
const ShieldIcon: React.FC<{ score?: number; className?: string }> = ({ score, className }) => (
...
export const SecurityScore: React.FC<SecurityScoreProps> = ({ defaultScore = 65, onLevelUp }) => {
  ...
  const [simulationResult, setSimulationResult] = useState<'safe' | 'scam' | null>(null);
  const [simulationAddress, setSimulationAddress] = useState('');

  // New states for sending tokens
  const [activeTab, setActiveTab] = useState<'send' | 'simulate' | 'monitor'>('send');
  const [sendAmount, setSendAmount] = useState('');
  const [sendAddress, setSendAddress] = useState('');
  const [transactionHistory] = useState([
    { type: 'blocked', address: '0x71C...9E3B', amount: '1.5 ETH', threat: 'Malicious Contract' },
    { type: 'safe', address: '0x8F2...1D4A', amount: '0.8 ETH', threat: null },
    { type: 'blocked', address: '0x3A9...7C2D', amount: '500 USDT', threat: 'Phishing Attempt' },
  ]);
  ...
  const nextLevelScore = score >= 90 ? 100 : score >= 75 ? 90 : score >= 60 ? 75 : score >= 40 ? 60 : 40;
          ...
          <div className="col-span-12 lg:col-span-4">
            ...
            >
              ...
              <div className="relative z-10 p-8 -mt-8">
                <div className={`flex flex-col items-center transition-all duration-700 transform ${isExpanded ? 'scale-95' : 'scale-100'}`}>
                  {/* Enhanced shield icon with glow effects */}
                  <div className={`relative transition-transform duration-700 ${isExpanded ? 'scale-90 -translate-y-4' : ''}`}>
                    ...
                    <div className="relative transform hover:scale-110 transition-all duration-500">
                      ...
                      <ShieldIcon score={score} className="w-40 h-40 transform hover:scale-105 transition-all duration-500" />
                    </div>
                  </div>
                ...
                <CardTitle className="flex items-center justify-between text-white px-4">
                  <div className="flex items-center space-x-3">
                    <div className="relative transform transition-transform group-hover:scale-110">
                      <Shield className={`h-8 w-8 text-cyan-400 transform transition-transform ${isRotating ? 'rotate-180' : ''}`} />
                  ...
                  {!loading && walletConnector.address && (
                    score >= 90 ? (
              ...
              <CardContent className="relative space-y-6 px-8">
                {loading ? (
                  <div className="flex items-center justify-center py-8">
                    ...
                    <p className="text-lg text-gradient-primary">Initializing neural shield...</p>
                  </div>
                ) : !walletConnector.address ? (
                  <div className="py-8 text-center">
                    <p className="text-lg text-gradient-primary">Connect wallet to activate neural shield</p>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="flex items-center justify-between bg-white/5 rounded-xl p-6 backdrop-blur-sm border border-white/10">
                      ...
                      <div className="text-right">
                        <div className="text-sm text-gradient-primary">Next Level</div>
                    ...
                    <div className={`space-y-3 transform transition-all duration-500 ${showDetails ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                      <div className="flex justify-between text-sm">
                        <span className="text-gradient-primary font-medium">Neural Shield Capacity</span>
                        <span className="text-cyan-400 font-bold">{Math.round(progressToNext)}%</span>
                      </div>
                    ...
                    <div className={`grid grid-cols-2 gap-4 transform transition-all duration-500 ${showDetails ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                      <div className="flex items-center space-x-3 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 border border-white/10 backdrop-blur-sm group">
                        ...
                        <div>
                          ...
                          <div className="text-white font-medium text-lg">+{securityBreakdown.threatsBlocked}</div>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 border border-white/10 backdrop-blur-sm group">
                        ...
                        <div>
                          <span className="text-gray-400 text-sm">DAO Votes</span>
                          <div className="text-white font-medium text-lg">+{securityBreakdown.daoVotes}</div>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 border border-white/10 backdrop-blur-sm group">
                        ...
                        <div>
                          <span className="text-gray-400 text-sm">Reports Filed</span>
                          <div className="text-white font-medium text-lg">+{securityBreakdown.reports}</div>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 border border-white/10 backdrop-blur-sm group">
                        ...
                        <div>
                          ...
                          <div className="text-white font-medium text-lg">+{securityBreakdown.walletAge}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className={`absolute inset-0 transition-all duration-700 ${
                      ...
                      <div className="h-full flex flex-col justify-center items-center p-8 backdrop-blur-md bg-black/40">
                        ...
                        <div className="w-full space-y-5">
                          {[
                            { label: 'Smart Contract Safety', value: 90, color: 'from-indigo-500 to-violet-500' },
                            { label: 'Transaction Security', value: 85, color: 'from-fuchsia-500 to-pink-500' },
                            { label: 'Wallet Protection', value: 88, color: 'from-cyan-500 to-blue-500' }
                          ].map((metric) => (
                        ...
                        {isSimulating ? (
                          <div className="space-y-4">
                            ...
                            <div className="space-y-2 text-center">
                              <p className="text-lg font-medium text-white/90">Neural Shield Analysis</p>
                              <p className="text-white/60 text-sm">Scanning for threats...</p>
                            </div>
                          </div>
                        ) : simulationResult ? (
                          <div className="text-center space-y-4">
                            {simulationResult === 'safe' ? (
                              <>
                                ...
                                <div className="space-y-2">
                                  <p className="text-lg font-medium text-emerald-400">Transaction Safe</p>
                              ...
                              <>
                                ...
                                <div className="space-y-2">
                                  <p className="text-lg font-medium text-red-400">Threat Detected!</p>
                                  <p className="text-white/60 text-sm max-w-[80%] mx-auto">
                                    Malicious contract behavior identified. Transaction blocked.
                                  </p>
                                  <div className="text-emerald-400 text-sm">+3 Shield Points Earned!</div>
                                </div>
                              </>
                            )}
                          </div>
                          ...
                          <div className="text-center space-y-4">
                            ...
                            <div className="space-y-2">
                              <p className="text-lg font-medium text-white/90">Simulate Threat</p>
                              <p className="text-white/60 text-sm max-w-[80%] mx-auto">
                  ...
                  <div className="space-y-6 animate-fade-in-up">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-gradient-to-br from-indigo-500/10 to-fuchsia-500/10 rounded-lg p-4 border border-white/10">
                        ...
                        <p className="text-white/90 font-medium">Neural Shield Active</p>
                      </div>
                      <div className="bg-gradient-to-br from-fuchsia-500/10 to-cyan-500/10 rounded-lg p-4 border border-white/10">
                        <div className="flex items-center space-x-2 mb-2">
                          <Brain className="w-5 h-5 text-fuchsia-400" />
                          <span className="text-white/60 text-sm">AI Status</span>
                        </div>
                        <p className="text-white/90 font-medium">Protection Online</p>
                      </div>
                    </div>

                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <h4 className="text-lg font-medium text-white/90">Recent Activity</h4>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  <div className="text-sm font-medium text-white/90">{tx.address}</div>
                                  <div className="text-sm text-white/60">{tx.amount}</div>
                                </div>
                              </div>
                              {tx.threat && (
